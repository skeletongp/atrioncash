...
<body class="legal">
    ...
    <section class=" mx-auto px-12 py-24  bg-white">
        <article class="mx-2 border-double   border-l-2 border-r-2 border-red-500 px-8">
            ...
            <p>
                En la ciudad de <b>{{ $contrato->notario->municipio->nombre }},
                    {{ $contrato->notario->municipio->provincia->nombre }}</b>, a los <b>{{ $dia }}
                    ({{ date_format($date, 'd') }}) días
                    del mes de {{ $mes }} del año {{ $año }} ({{ date_format($date, 'Y') }})</b>,
                por
                ante mí, <b
                    class="uppercase">{{ $contrato->notario->titulo . ' ' . $contrato->notario->name }}</b>,
                Abogado
            ...
            <p>
                ...
                misma fecha, a razón de un <b class="uppercase">{{ $f->format($contrato->deuda->interes) }} por
                    ciento ({{ $contrato->deuda->interes }}%)</b> de interés {{ $contrato->deuda->periodicidad }}
                sobre el saldo pendiente, hasta el
            ...
            <p> Además, queda por entendido que, a la falta del pago del presente pagaré, se hará exigible la garantía
                ...
                términos,
                las partes <b>DEUDORA</b> y <b>ACREEDORA</b> declaran, que, si al vencimiento del presente préstamo no
                se ha concluido
                con
                lo previsto en el mismo, la parte <b>DEUDORA</b> pagará un <b class="uppercase">cinco por ciento
                    (5%)</b> por el mes de retardo sobre el
                capital absoluto acreditado.<b>EL DEUDOR</b>, consiente y su <b>ACREEDOR</b> acepta que deberá liquidar
                la
                obligación
                ...
                <b class="uppercase">Mil
                    doscientos veintiséis (1226)</b> del <b>Código Civil Dominicano</b>, el cual establece que la
                cláusula penal es
                aquella
            ...
            {{-- Coletilla final --}}
            <div class="firma mt-16 pt-6"></div>
            @if ($contrato->parrafo)
                <p>
                    <b class="uppercase"> Párrafo: </b> {{ $contrato->parrafo }}
                </p>
            @endif
            <p>
                ...
                procedido a firmarlo junto conmigo y ante mí, notario público infrascrito que <b
                    class="uppercase">certifico y doy fe</b> ha
                sido el
                presente documento firmado y rubricado de buena fe, en dos originales de un mismo tenor y efecto
                <b>ACREEDOR</b> y
                <b>DEUDOR</b>, en la Ciudad de {{ $contrato->notario->municipio->nombre }},
                {{ $contrato->notario->municipio->provincia->nombre }}, República Dominicana, a los
                <b>{{ $dia }}
                    ({{ date_format($date, 'd') }}) días
                    del mes de {{ $mes }} del año {{ $año }} ({{ date_format($date, 'Y') }})</b>
            ...
            <div class="text-center flex flex-col items-center mt-6 uppercase">
                <span class=" border-b-2 border-black font-bold">
                    {{ $contrato->notario->titulo . ' ' . $contrato->notario->name }}</span>
